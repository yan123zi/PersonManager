                            ...
                            <div class="page-title">
                                <ol class="breadcrumb">
                                    <li class="breadcrumb-item">
	                                    <a href="${pageContext.request.contextPath }/adminTrain/trainInfoPage">
		                                    <button type="button" class="btn btn-primary btn-flat btn-addon m-b-10 m-l-5">
		                                    <i class="ti-plus"></i>发布培训信息</button>
	                                    </a>
                                    </li>
                                </ol>
                            </div>
   								...
   								<div class="card-body">
                                    <div class="table-responsive">
                                        <table class="table table-hover ">
                                            ...
                                            <tbody>
	                                            <c:forEach items="${pageInfo.list }" var="training">
	                                                <tr>
	                                                    <th>${training.tPlan }</th>
	                                                    <td>${training.dName }</td>
	                                                    <td>${training.pPosition }</td>
	                                                    <td><fmt:formatDate type="date" value="${training.tDate }"/></td>
	                                                    <td>${training.tMan }</td>
	                                                    <td>
	                                                    	 <a href="${pageContext.request.contextPath }/adminTrain/trainInfo/${training.tId }"><button type="button" class="btn btn-success btn-outline btn-rounded m-b-10 m-l-5">查看/编辑</button></a>
	                                                    	 <button type="button" del="${training.tId }" data-toggle="modal" data-target="#d" class="btn btn-danger btn-outline btn-rounded m-b-10 m-l-5 delete">删除</button>
	                                                    </td>
	                                                </tr>
	                                             </c:forEach>
                                            </tbody>
						  ...
						  <div class="modal-dialog" role="document">
						    <div class="modal-content">
						      <div class="modal-header">
						        <h4 class="modal-title" id="myModalLabel">是否删除该培训信息?</h4>
						        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
						      </div>
						      <div class="modal-body">
						      	<form:form id="delete" method="delete" action="${pageContext.servletContext.contextPath }/adminTrain/trainInfoDelete">
                            ...
                            <div style="margin-top: 20px;float: right;font-size: 20px;">
	                            <nav aria-label="Page navigation">
								  <ul id="page" class="pagination">				
								  	<li style="text-align: center;"><a href="${pageContext.request.contextPath }/adminTrain/allTraining">首页</a></li>
								    <li style="text-align: center;">
								    	<c:if test="${pageInfo.hasPreviousPage }">
									      <a href="${pageContext.request.contextPath }/adminTrain/allTraining?pn=${pageInfo.pageNum-1}" aria-label="Previous">
								    ...
								    <c:forEach var="i" begin="1" end="${pageInfo.pages }">
									    ...
									    <c:if test="${i!=pageInfo.pageNum }">
									    	<li style="text-align: center;"><a href="${pageContext.request.contextPath }/adminTrain/allTraining?pn=${i }">${i }</a></li>
									    </c:if>
								    </c:forEach>
								    <li style="text-align: center;">
								    	<c:if test="${pageInfo.hasNextPage }">
									      <a href="${pageContext.request.contextPath }/adminTrain/allTraining?pn=${pageInfo.pageNum+1}" aria-label="Next">
									        <span aria-hidden="true">&raquo;</span>
									      </a>
									    </c:if>
								    </li>
								    
								    <li style="text-align: center;"><a href="${pageContext.request.contextPath }/adminTrain/allTraining?pn=${pageInfo.pages}">末页</a></li>
								  </ul>
								</nav>	
                       		 </div>
          </div>
